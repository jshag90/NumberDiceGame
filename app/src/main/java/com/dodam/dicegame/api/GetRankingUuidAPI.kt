...
fun getRankingUuidWithOkHttpAsync(
    context: Context,
    onResult: (RankingDto?) -> Unit
) {
    ...
    CoroutineScope(Dispatchers.IO).launch {
        try {
            val url = "$serverUrl/score/ranking/uuid=$uuid"
            ...
            val returnCodeVO: ReturnCodeVO<RankingDto>? = fromJson(responseBody)
